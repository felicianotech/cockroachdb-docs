...
To check this, open the Admin UI at <a href="http://localhost:8080" data-proofer-ignor>http://localhost:8080</a>, click **View nodes list** at the right, and check the the replica count is the same on all nodes.
...
3. Use the [`cockroach zone`](../stable/configure-replication-zones.html) command to create a replication zone for the `startrek` database, forcing all the data in the database to be located on EU-based nodes:
...
4. Use the [`cockroach zone`](../stable/configure-replication-zones.html) command to create a distinct replication zone for the `intro` database, forcing all the data in the database to be located on US-based nodes:
...
Now verify that the data for the table in the `intro` database is be located on US-based nodes, and the data for the tables in the `startrek` database is located on EU-based nodes.
